...
async function cargarProgresoDesdeFirebase() {
  try {
    ...
    if (doc.exists) {
      const data = doc.data();
      const fechaGuardada = (data.fecha || "").trim();
      const progresoGuardado = data.progreso;
      console.log("[DEBUG] Fecha guardada en Firebase:", fechaGuardada);
      console.log("[DEBUG] Progreso guardado en Firebase:", progresoGuardado);
      console.log("[DEBUG] Comparando fechas:", fechaGuardada, fechaActual);
      console.log("[DEBUG] Igualdad:", fechaGuardada === fechaActual);

      // Si es un nuevo día, reinicia progreso en Firebase y localStorage
      if (fechaGuardada !== fechaActual) {
        ...
        console.log("[DEBUG] Reinicio diario aplicado.");
      } else {
        // Si es el mismo día, sincroniza localStorage con Firebase
        progreso = progresoGuardado;
        localStorage.setItem("progreso", progresoGuardado);
        localStorage.setItem("fecha", fechaActual);
        console.log("[DEBUG] localStorage sincronizado con Firebase.");
      }
    } else {
      ...
      localStorage.setItem("progreso", 0);
      localStorage.setItem("fecha", fechaActual);
      console.log("[DEBUG] Documento Firebase inicializado.");
    }
    ...
    let fechaActual = obtenerFechaActual();
    let fechaGuardadaLocal = (localStorage.getItem("fecha") || "").trim();
    let progresoGuardadoLocal = localStorage.getItem("progreso");
    console.log("[DEBUG] Fecha guardada en localStorage:", fechaGuardadaLocal);
    console.log("[DEBUG] Progreso guardado en localStorage:", progresoGuardadoLocal);
    console.log("[DEBUG] Comparando fechas (catch):", fechaGuardadaLocal, fechaActual);
    console.log("[DEBUG] Igualdad (catch):", fechaGuardadaLocal === fechaActual);

    if (fechaGuardadaLocal !== fechaActual) {
      progreso = 0;
      localStorage.setItem("progreso", 0);
      localStorage.setItem("fecha", fechaActual);
      console.log("[DEBUG] (catch) Reinicio diario aplicado.");
    } else {
      progreso = parseInt(localStorage.getItem("progreso")) || 0;
      console.log("[DEBUG] (catch) localStorage sincronizado.");
    }
    actualizarContadorDisplay();
  }
}
...
comidaButton.addEventListener("click", async () => {
  // Verifica la fecha antes de sumar progreso
  let fechaActual = obtenerFechaActual();
  let fechaGuardadaLocal = (localStorage.getItem("fecha") || "").trim();
  if (fechaGuardadaLocal !== fechaActual) {
    progreso = 0;
  ...
  if (progreso < 100) {
    ...
    actualizarContadorDisplay();
    try {
      await counterRef.set({ progreso: progreso, fecha: fechaActual });
    } catch (error) {
...
// Verifica y reinicia progreso automáticamente al mostrar la página
window.addEventListener("focus", () => {
  cargarProgresoDesdeFirebase();
});
cargarProgresoDesdeFirebase();
// Verifica y reinicia progreso automáticamente cada minuto
setInterval(() => {
  cargarProgresoDesdeFirebase();
...
flechaIzq.addEventListener("click", () => {
  indiceActual =
    (indiceActual - 1 + imagenesGaleria.length) % imagenesGaleria.length;
